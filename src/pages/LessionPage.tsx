...
const LessonPage = () => {
  ...
  const [classData] = useState<ClassDetails | null>(null);

  useEffect(() => {
    const fetchLessonData = async () => {
      try {
        setIsLoading(true);
        const data = await lessonsService_dev.getLesson(
          params.clsid!,
          params.id!
        );
        setLessonData(data);
  ...
  const url =
    "https://us-central1-dopamine-lite-b61bf.cloudfunctions.net/getPresignedUrl?manifest_key=index.m3u8&segment_keys=index0.ts,index1.ts&folder=kana&expiration=3600";
          ...
          <VStack align="stretch" gap={6} mb={8} w="full">
            ...
            <HStack gap={8} flexWrap="wrap" color="gray.600">
              <HStack>
                ...
                <Text fontSize="0.875rem" color={Light.black75}>
                  Created:
                </Text>
                <Text fontSize="0.875rem" color={Light.black100}>
            ...
            >
              <HStack align="flex-start" maxW={{ base: "100%", md: "60%" }}>
                ...
                <VStack align="stretch">
                  <Text fontSize="1.125rem" color={Light.black100}>
                    About this lesson
                  </Text>
                  <Text fontSize="0.9375rem" color={Light.black75}>
