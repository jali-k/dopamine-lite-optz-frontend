...
const AdminClassesPage = () => {
  ...
  const fetchClasses = async () => {
    try {
      const data: Class[] = await classesService.getClasses();
      console.log(data);
      setClasses(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      if (selectedClass) {
        await classesService.updateClass(selectedClass.classId, formData);
        toaster.create({
  ...
  function onClassClick(cls: Class): void {
    navigate(`/admin/classes/${cls.classId}/lessons`);
