...
export default function AdminCreateAccessGroupPage() {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
   e.preventDefault();
   try {
     await accessGroupService_dev.createGroup(formData);
     toaster.create({ title: 'Group created successfully', type: 'success' });
     navigate('/admin/access-groups');
   } catch (error) {
     toaster.create({ title: 'Error creating group', type: 'error' });
   }
 };
       ...
       <VStack gap={8} align="stretch">
         <Card.Root p={6}>
           <VStack align="stretch" gap={6}>
             <Heading color={colors.primary} size="lg">Create Access Group</Heading>
