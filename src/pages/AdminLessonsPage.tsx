...
const AdminLessonsPage = () => {
  ...
  const [lessons, setLessons] = useState<ClassDetails>();
  ...
  const fetchLessons = async () => {
    const response = await lessonsService
      .getLessonsByClassId(params.clsid || "", "dasun.theekshana.git@gmail.com")
      .then((data) => {
        console.log(data);
        return data;
      });
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this lesson?")) {
      try {
        await lessonsService.deleteLesson(classId!, "");
        toaster.create({
  ...
  const handleEditClick = (lessonId: number) => {
                    ...
                    <VStack align="start" gap={4}>
                      ...
                      <HStack justify="flex-end" w="full" gap={2}>
                        <IconButton
                          ...
                          onClick={() => handleEditClick(lesson.classId)}
                          colorScheme="blue"
                          variant="ghost"
                        >
                          <FaEdit />
                        ...
                        <IconButton
                          ...
                          onClick={() => handleDelete(lesson.classId)}
                          colorScheme="red"
                          variant="ghost"
                        >
                          <FaTrash />
                  ...
                  <CardBody>
                    <VStack align="start" gap={4}>
                      ...
                      <HStack justify="flex-end" w="full" gap={2}>
                        <IconButton
                          aria-label="Edit lesson"
                          onClick={() => handleEditClick(note.classId)}
                          colorScheme="blue"
                          variant="ghost"
                        >
                          <FaEdit />
                        ...
                        <IconButton
                          aria-label="Delete lesson"
                          onClick={() => handleDelete(note.classId)}
                          colorScheme="red"
                          variant="ghost"
                        >
                          <FaTrash />
